...
def test_company_source_requires_a_verified_company_domain_not_a_generic_ir_hostname():
    """SELECT INVARIANT: IR-looking generic domains are not company evidence."""
    assert classify_source_kind("https://investor.com/company-update") == "secondary"
    assert classify_source_kind("https://ir.com/earnings-release") == "secondary"


def test_company_source_is_bound_to_the_supported_target_company_domain():
    """SELECT INVARIANT: exact target-domain IR hosts and paths are company evidence."""
    assert (
        classify_source_kind(
            "https://investor.nvidia.com/news/default.aspx",
            tickers=["NVDA"],
        )
        == "company"
    )
...
def test_acquisition_isolates_one_publisher_fetch_failure_and_records_safe_outcome():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        records_by_url = {
            record.article.original_url: record for record in storage.list_article_records()
        }
        assert records_by_url[failed_url].content_status == "failed"
        assert records_by_url[failed_url].article.content_validation_reason == "http_429"
        assert records_by_url[normalized_failure_url].content_status == "failed"
        assert (
            records_by_url[normalized_failure_url].article.content_validation_reason
            == "fetch_exception"
        )
        storage.close()


def test_acquisition_isolates_one_search_intent_failure_and_continues_other_intents():
    with tempfile.TemporaryDirectory() as tmpdir:
        storage = SQLiteNewsStore(db_path=os.path.join(tmpdir, "news.db"))
        storage.init_db()
        accepted_url = "https://www.reuters.com/technology/search-recovers"

        class OneIntentFails(FakeSearchProvider):
            def search(self, query: str, *, num_results: int):
                self.calls.append((query, num_results))
                if len(self.calls) == 1:
                    raise RuntimeError("search provider unavailable")
                return [SearchResult("Search recovers", accepted_url)]

        result = run_theme_research(
            ThemeResearchRequest(
                theme="AI infrastructure",
                analysis_goal="map bottlenecks",
                seed_query="first intent fails",
                db_path=os.path.join(tmpdir, "news.db"),
                research_root=os.path.join(tmpdir, "research"),
                local_retrieval_top_k=0,
                max_discovered_articles=1,
            ),
            storage=storage,
            local_vector_reader=FakeVectorStore(),
            search_provider=OneIntentFails({}),
            content_fetcher=FakeFetcher(
                {
                    accepted_url: {
                        "content": "Search recovery evidence about AI infrastructure bottlenecks.",
                        "canonical_url": accepted_url,
                    }
                }
            ),
        ...
        assert [item.url for item in result.evidence] == [accepted_url]
        assert result.evidence_provenance["acquisition"]["failure_codes"] == {
            "search_intent_failed": 1
        }
        storage.close()


def test_acquisition_isolates_reference_and_content_write_failures_per_result():
    with tempfile.TemporaryDirectory() as tmpdir:
        storage = SQLiteNewsStore(db_path=os.path.join(tmpdir, "news.db"))
        storage.init_db()
        reference_url = "https://publisher.example/reference-fails"
        write_url = "https://publisher.example/write-fails"
        accepted_url = "https://www.reuters.com/technology/accepted-after-storage-failures"
        provider = FakeSearchProvider(
            {
                "AI infrastructure map bottlenecks": [
                    SearchResult("Reference fails", reference_url),
                    SearchResult("Write fails", write_url),
                    SearchResult("Accepted after failures", accepted_url),
                ]
            }
        )
        fetcher = FakeFetcher(
            {
                write_url: {
                    "content": "Write failure article about AI infrastructure.",
                    "canonical_url": write_url,
                },
                accepted_url: {
                    "content": "Accepted evidence after isolated storage failures.",
                    "canonical_url": accepted_url,
                },
            }
        )
        original_create = storage.create_or_get_article_reference
        original_update = storage.update_article_content

        def create_reference(**kwargs):
            if kwargs["original_url"] == reference_url:
                raise RuntimeError("reference insert unavailable")
            return original_create(**kwargs)

        def update_content(article_id, **kwargs):
            if kwargs["original_url"] == write_url:
                raise RuntimeError("content write unavailable")
            return original_update(article_id, **kwargs)

        storage.create_or_get_article_reference = create_reference
        storage.update_article_content = update_content
        ...
        write_record = next(
            record
            for record in storage.list_article_records()
            if record.article.original_url == write_url
        )
        assert write_record.content_status == "failed"
        assert write_record.article.content_validation_reason == "content_write_failed"
        storage.close()
...
def test_theme_research_retries_failed_summary_on_later_run_before_accepting():
    """SELECT INVARIANT: ready content cannot bleach a persisted summary failure."""
    with tempfile.TemporaryDirectory() as tmpdir:
        storage = SQLiteNewsStore(db_path=os.path.join(tmpdir, "news.db"))
        storage.init_db()
        url = "https://www.reuters.com/technology/summary-recovers"

        class SummaryRecovers:
            def __init__(self):
                self.calls = 0
        ...
        provider = FakeSearchProvider(
            {
                "AI infrastructure monitor constraints": [
                    SearchResult("Summary recovers", url)
                ]
            }
        )
        fetcher = FakeFetcher(
            {
                url: {
                    "content": "Summary recovery evidence about infrastructure constraints.",
                    "canonical_url": url,
                }
            }
        )

        first = run_theme_research(
            request,
            storage=storage,
            local_vector_reader=FakeVectorStore(),
            search_provider=provider,
        ...
        assert first.evidence_provenance["acquisition"]["accepted"] == 0
        assert first.evidence_provenance["acquisition"]["failure_codes"] == {
            "summary_failed": 1
        }
        assert second.evidence_provenance["acquisition"]["accepted"] == 1
        assert second.evidence_provenance["acquisition"]["failed"] == 0
        assert record.summary_status == "ready"
        assert record.article.summary == "- Recovered summary"
        assert summarizer.calls == 2
        assert fetcher.calls == [url]
        storage.close()


def test_theme_research_retries_failed_generation_handoff_on_later_run():
    """SELECT INVARIANT: a ready row is re-offered until successor handoff succeeds."""
    with tempfile.TemporaryDirectory() as tmpdir:
        storage = SQLiteNewsStore(db_path=os.path.join(tmpdir, "news.db"))
        storage.init_db()
        url = "https://www.reuters.com/technology/handoff-recovers"
        handoff_calls = []
        ...
        provider = FakeSearchProvider(
            {
                "AI infrastructure monitor constraints": [
                    SearchResult("Handoff recovers", url)
                ]
            }
        )
        fetcher = FakeFetcher(
            {
                url: {
                    "content": "Handoff recovery evidence about infrastructure constraints.",
                    "canonical_url": url,
                }
            }
        )
        ...
        article_id = storage.list_article_records()[0].id
        assert first.evidence_provenance["acquisition"]["accepted"] == 0
        assert first.evidence_provenance["acquisition"]["failure_codes"] == {
            "generation_handoff_failed": 1
        }
        assert second.evidence_provenance["acquisition"]["accepted"] == 1
        assert second.evidence_provenance["acquisition"]["generation_handoff"] == {
            "scheduled": 1,
...
def test_theme_research_isolates_summary_failure_marker_error():
    """SELECT INVARIANT: failure-state persistence errors cannot abort the whole theme run."""
    with tempfile.TemporaryDirectory() as tmpdir:
        storage = SQLiteNewsStore(db_path=os.path.join(tmpdir, "news.db"))
        storage.init_db()
        first_url = "https://www.reuters.com/technology/summary-marker-fails"
        second_url = "https://www.reuters.com/technology/summary-still-runs"
        original_marker = storage.mark_article_summary_failure

        def marker(article_id, content_sha256):
            if article_id == 1:
                raise RuntimeError("summary status database unavailable")
            return original_marker(article_id, content_sha256)

        storage.mark_article_summary_failure = marker
        class FirstSummaryFails:
            def summarize(self, article):
                if article.title == "First":
                    raise RuntimeError("summary failed")
                return "Second summary succeeds."

        summarizer = FirstSummaryFails()

        result = run_theme_research(
            ThemeResearchRequest(
                theme="AI infrastructure",
                analysis_goal="monitor suppliers",
                db_path=os.path.join(tmpdir, "news.db"),
                research_root=os.path.join(tmpdir, "research"),
                seed_query="AI infrastructure monitor suppliers",
                local_retrieval_top_k=0,
            ),
            storage=storage,
            search_provider=FakeSearchProvider(
                {
                    "AI infrastructure monitor suppliers": [
                        SearchResult(title="First", url=first_url, snippet="first"),
                        SearchResult(title="Second", url=second_url, snippet="second"),
                    ]
                }
            ),
            content_fetcher=FakeFetcher(
                {
                    first_url: {"content": "First article content.", "canonical_url": first_url},
                    second_url: {"content": "Second article content.", "canonical_url": second_url},
                }
            ),
            summarizer=summarizer,
            local_vector_reader=FakeVectorStore(),
        ...
        assert result.evidence[0].title == "Second"
        assert result.evidence_provenance["acquisition"]["failed"] == 1
        assert result.evidence_provenance["acquisition"]["failure_codes"] == {
            "summary_failed": 1,
            "summary_failure_persist_failed": 1,
        }
        first = storage.get_article_record(1)
        assert first is not None
        assert first.summary_status == "pending"
        storage.close()
