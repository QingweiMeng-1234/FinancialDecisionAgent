...
def test_stage3_admits_watch_segment_to_stage4_with_unresolved_dimensions(tmp_path):
    """A researchable Segment may advance while non-gating gaps stay explicit."""

    class PartialWatchScorer:
        def assess(self, node, claims, evidence_cards, *, request, contract_version):
            if not evidence_cards:
                return _unknown_score(node.node_id)
            claims_by_id = {claim.claim_id: claim for claim in claims}
            evidence_by_dimension = {
                claims_by_id[card.claim_id].primary_scoring_dimension: (
                    card.evidence_id,
                    card.claim_id,
                )
                for card in evidence_cards
            ...
            unknown = {
                item.dimension: item
                for item in _unknown_score(node.node_id).dimensions
            }
            dimensions = tuple(
                _supported_dimension(name, *evidence_by_dimension[name])
            ...
            return SegmentScoreDraft(
                ...
                missing_material_fields=tuple(
                    name for name in DIMENSIONS if name not in supported_names
                ),
                demand_direct_evidence=True,
                supply_direct_evidence=True,
                counter_evidence_search_complete=False,
    ...
    class PreserveDirectEvidenceCritic:
        def apply(self, *, ordinal_draft, **kwargs):
            return ordinal_draft

    repository = ThemeChokepointRepository(tmp_path / "theme.db")
    request = _seed_run(repository, request=_request(max_iterations=1))

    result = _controlled_loop(
        repository,
        RecordingAcquirer(_all_dimension_candidates()),
        PartialWatchScorer(),
        critic=PreserveDirectEvidenceCritic(),
    ).run(request.run_id)

    assessment = result.assessments[0]
    assert assessment.primary_state == "watch_segment"
    assert any(
