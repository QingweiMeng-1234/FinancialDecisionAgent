...
class OverlapRecordingFetcher:
    ...
    def fetch(self, url):
        ...
        self._second_started.wait(timeout=0.5)
        time.sleep(0.02)
        with self._lock:
            self.active -= 1
        name = "Alpha" if "alpha" in url else "Beta"
        return FetchResult(url, url, f"{name} publisher report details. " * 20)
...
def test_ingest_raw_inputs_fetches_different_publishers_concurrently(temp_storage_dir):
    db_path, _ = temp_storage_dir
    storage = SQLiteNewsStore(db_path=db_path)
    storage.init_db()
    fetcher = OverlapRecordingFetcher()
    ...
    result = ingest_raw_inputs(
        ...
        content_fetcher=fetcher,
        publisher_fetch_workers=2,
    )

    assert fetcher.max_active == 2
    assert result.accepted_inputs == 2
    assert [item.input_index for item in result.items] == [0, 1]


def test_completed_publisher_fetch_commits_before_slower_future_finishes(temp_storage_dir):
    """SELECT INVARIANT: one slow URL cannot hold completed canonical content pending."""
    db_path, _ = temp_storage_dir
    ...
    class FastAndBlockedFetcher:
        def fetch(self, url):
            if "fast" in url:
                fast_returned.set()
                return FetchResult(url, url, "Fast publisher report details. " * 20)
            slow_started.set()
            release_slow.wait(timeout=5)
            return FetchResult(url, url, "Slow publisher report details. " * 20)
    ...
    def worker():
        ...
        def update_and_signal(article_id, **kwargs):
            changed = original_update(article_id, **kwargs)
            if kwargs["title"] == "Fast report":
                fast_committed.set()
            return changed

        storage.update_article_content = update_and_signal
        try:
            ingest_raw_inputs(
                [
                    RawEventInput(
                        source="news",
                        raw_text="Fast description",
                        title="Fast report",
                ...
                storage,
                summarizer=FakeSummarizer(),
                content_fetcher=FastAndBlockedFetcher(),
                publisher_fetch_workers=2,
            )
    ...
    try:
        ...
        assert fast_committed.wait(timeout=1)
        observer = SQLiteNewsStore(db_path=db_path)
        observer.init_db()
        statuses = {
            record.article.title: record.content_status
            for record in observer.list_article_records()
        }
        observer.close()
        assert statuses == {"Fast report": "ready", "Slow report": "pending"}
    finally:
        release_slow.set()
...
@pytest.mark.parametrize("alpha_delay,beta_delay", [(0.0, 0.05), (0.05, 0.0)])
def test_canonical_collision_has_stable_survivor_and_content_across_fetch_order(
    tmp_path, alpha_delay, beta_delay
):
    """SELECT INVARIANT: network timing cannot choose canonical identity or payload."""
    storage = SQLiteNewsStore(db_path=str(tmp_path / "news.db"))
    storage.init_db()
    canonical_url = "https://publisher.example/canonical-story"
    ...
    class RedirectingFetcher:
        def fetch(self, url):
            if "alpha" in url:
                ...
                return FetchResult(
                    url, canonical_url, "Alpha canonical publisher details. " * 20
                )
            time.sleep(beta_delay)
            return FetchResult(
                url, canonical_url, "Beta canonical publisher details. " * 20
            )
    ...
    result = ingest_raw_inputs(
        ...
        storage,
        summarizer=FakeSummarizer(),
        content_fetcher=RedirectingFetcher(),
        publisher_fetch_workers=2,
    )

    records = storage.list_article_records()
    assert len(records) == 1
    assert records[0].id == 1
    assert records[0].article.title == "Alpha report"
    assert records[0].article.content.startswith("Alpha canonical publisher details")
    assert [item.article_id for item in result.items] == [1, 1]
    storage.close()


def test_canonical_merge_records_and_completes_retired_artifact_cleanup(tmp_path):
    """SELECT INVARIANT: retiring a completed provisional ID leaves no untracked vector or file."""
    storage = SQLiteNewsStore(db_path=str(tmp_path / "news.db"))
    storage.init_db()
    canonical_url = "https://publisher.example/cleanup-story"

    class BetaFirstFetcher:
        def fetch(self, url):
            if "alpha" in url:
                time.sleep(0.05)
                return FetchResult(url, canonical_url, "Alpha cleanup details. " * 20)
            return FetchResult(url, canonical_url, "Beta cleanup details. " * 20)
    ...
    result = ingest_raw_inputs(
        [
            RawEventInput(
                source="news", raw_text="Alpha", title="Alpha", description="Alpha",
                url="https://alpha.example/story", published_at=datetime.now(),
            ),
            RawEventInput(
                source="news", raw_text="Beta", title="Beta", description="Beta",
                url="https://beta.example/story", published_at=datetime.now(),
            ),
        ],
        storage,
        vector_store=vectors,
        summarizer=FakeSummarizer(),
        content_fetcher=BetaFirstFetcher(),
        publisher_fetch_workers=2,
    )

    receipt = storage.conn.execute(
        "SELECT * FROM article_identity_merges WHERE retired_article_id = 2"
    ).fetchone()
    assert [item.article_id for item in result.items] == [1, 1]
    assert vectors.deleted == [2]
    assert receipt["survivor_article_id"] == 1
    assert receipt["vector_cleanup_status"] == "deleted"
    assert receipt["content_cleanup_status"] == "deleted"
    assert not os.path.exists(receipt["retired_content_path"])
    storage.close()


def test_canonical_merge_preserves_previous_ready_content_when_lower_refresh_fails(tmp_path):
    """SELECT INVARIANT: a failed lower-ID refresh cannot destroy the completed duplicate it replaces."""
    storage = SQLiteNewsStore(db_path=str(tmp_path / "news.db"))
    storage.init_db()
    canonical_url = "https://publisher.example/fallback-story"

    class BetaFirstFetcher:
        def fetch(self, url):
            if "alpha" in url:
                time.sleep(0.05)
                return FetchResult(url, canonical_url, "Alpha fallback details. " * 20)
            return FetchResult(url, canonical_url, "Beta usable details. " * 20)

    original_update = storage.update_article_content

    def fail_lower_update(article_id, **kwargs):
        if article_id == 1 and kwargs.get("canonical_url") == canonical_url:
            raise RuntimeError("lower canonical commit failed")
        return original_update(article_id, **kwargs)

    storage.update_article_content = fail_lower_update
    ingest_raw_inputs(
        [
            RawEventInput(
                source="news", raw_text="Alpha", title="Alpha", description="Alpha",
    ...
    survivor = storage.get_article_record(1)
    assert survivor is not None
    assert survivor.content_status == "ready"
    assert survivor.article.content.startswith("Beta usable details")
    assert survivor.article.canonical_url == canonical_url
    storage.close()


def test_unchanged_canonical_merge_still_cleans_retired_identity(tmp_path):
    """SELECT INVARIANT: the unchanged fast path cannot bypass canonical merge cleanup."""
    storage = SQLiteNewsStore(db_path=str(tmp_path / "news.db"))
    storage.init_db()
    canonical_url = "https://publisher.example/identical-story"
    identical = "Identical canonical publisher details. " * 20

    class BetaFirstFetcher:
        def fetch(self, url):
            if "alpha" in url:
                time.sleep(0.05)
            return FetchResult(url, canonical_url, identical)
    ...
    ingest_raw_inputs(
        [
            RawEventInput(source="news", raw_text="Alpha", title="Alpha", description="Alpha",
                          url="https://alpha.example/story", published_at=datetime.now()),
            RawEventInput(source="news", raw_text="Beta", title="Beta", description="Beta",
                          url="https://beta.example/story", published_at=datetime.now()),
        ],
        storage,
        vector_store=vectors,
    ...
    survivor = storage.get_article_record(1)
    assert vectors.added == [2]
    assert vectors.deleted == [2]
    assert receipt["vector_cleanup_status"] == "deleted"
    assert receipt["content_cleanup_status"] == "deleted"
    assert receipt["completed_at"] is not None
    assert survivor is not None
    assert os.path.normpath(survivor.article.content_path).split(os.sep)[-2] == "1"
    storage.close()


def test_pending_canonical_merge_cleanup_is_retried_on_later_ingestion(tmp_path):
    """SELECT INVARIANT: a pending merge receipt is a durable retry source, not a lost local variable."""
    storage = SQLiteNewsStore(db_path=str(tmp_path / "news.db"))
    storage.init_db()
    canonical_url = "https://publisher.example/retry-cleanup-story"

    class BetaFirstFetcher:
        def fetch(self, url):
            if "alpha" in url:
                time.sleep(0.05)
            text = "Alpha retry details. " if "alpha" in url else "Beta retry details. "
            return FetchResult(url, canonical_url, text * 20)
    ...
    pending = storage.conn.execute(
        "SELECT completed_at FROM article_identity_merges WHERE retired_article_id = 2"
    ).fetchone()
    ...
    completed = storage.conn.execute(
        "SELECT * FROM article_identity_merges WHERE retired_article_id = 2"
    ).fetchone()
    assert vectors.deleted == [2]
