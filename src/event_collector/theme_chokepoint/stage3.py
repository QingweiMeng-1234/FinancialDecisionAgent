...
STAGE4_ADMISSIBLE_SEGMENT_STATES = {
    "watch_segment",
...
class EvidenceChokepointLoop:
    ...
    def run_candidate(self, run_id: str, *, execution) -> Stage3Result:
        ...
        Evidence/scoring slice only. Existing critic/relief adapters may persist
        base-run lineage or spend outside the RPC budget, so they are not reused
        implicitly. This is not the paired-ablation/promotion API.
        """
        if self.critic is not None or self.relief_provider is not None:
            raise ValueError("RPC shadow requires candidate-scoped critic/relief adapters; not yet supported")
        if Path(self.repository.db_path).resolve() == execution.path:
            raise ValueError("RPC candidate database must be separate from canonical database")
        return self._run(run_id, execution=execution)

    def _run(self, run_id: str, *, execution=None) -> Stage3Result:
        ...
        if execution is not None:
            execution.prepare(run, graph, contract_identity={
                ...
                "scorer": f"{type(self.scorer).__module__}.{type(self.scorer).__qualname__}",
            })
            cached = execution.get_result()
        ...
        while self._missing_pairs(nodes, drafts):
            ...
            if execution is not None:
                batch = execution.acquire_round(
                    ...
                    query_builder=self._targeted_query,
                )
                # Accepted outputs are already individually durable. Merge in
                # planner order and reuse the original materializer/scorer.
                for candidate in batch.candidates:
                    key = _candidate_fact_key(candidate)
                    if key not in fact_keys and not any(
        ...
        stage4_admissible = any(
            item.primary_state in STAGE4_ADMISSIBLE_SEGMENT_STATES
            for item in assessments
        )
        if stage4_admissible:
            # Once Stage 3 has identified a researchable Segment, unresolved or
            # stale dimensions remain visible as uncertainty but do not block
            # company-level research in Stage 4.
            ...
            incomplete_reasons = ()
        save_result = (
            self.repository.save_stage3_result if execution is None else execution.save_result
        )
        return save_result(
            run_id,
            status=status,
