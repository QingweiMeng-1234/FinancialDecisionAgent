...
Models extract scoped facts.  This module owns the mechanical boundary mapping for
the four v1.5 dimensions whose semantics changed after v1.4.
"""
...
class V15ScoringContract:
    """Hash-pinned reader for the unfrozen v1.5 semantic task contract."""
    ...
    def __init__(self, path: str | Path, *, expected_sha256: str) -> None:
        ...
        if actual != expected_sha256:
            raise ValueError(
                "v1.5 executable scoring contract SHA-256 mismatch: "
                f"expected={expected_sha256} actual={actual}"
            )
        try:
            payload = json.loads(raw.decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError) as error:
            raise ValueError("v1.5 executable scoring contract is invalid JSON") from error
        if payload.get("contract_id") != "theme-chokepoint-semantic-task-contract-v1.5":
            raise ValueError("v1.5 executable scoring contract ID is not canonical")
        if payload.get("status") != "freeze_candidate":
            raise ValueError("v1.5 executable scoring contract is not a freeze candidate")
        if payload.get("inherits_scoring_contract") != "theme-chokepoint-scoring-v1.5":
            raise ValueError("v1.5 human scoring contract lineage is invalid")
        weights = payload.get("segment_weights", {})
        if not isinstance(weights, dict) or sum(weights.values()) != 100:
            raise ValueError("v1.5 Segment weights must sum to 100")
        ...
        if set(semantics) != required:
            raise ValueError("v1.5 Segment semantics are incomplete")
