# Theme Chokepoint Scoring Contract v1.6

## 1. 状态与继承

| 字段 | 值 |
| --- | --- |
| contract_id | `theme-chokepoint-scoring-v1.6` |
| 状态 | `freeze_candidate`（2026-08-23） |
| 基础合同 | v1.5未被本文件明确替换的Bound Basis、Coverage、Hard Gate与状态优先级 |
| 本版语义变化 | Qualification Barrier新增“精确时长或公开可观察流程”的双路径评分；其余Segment语义继承v1.5 |
| 机器合同 | `tools/theme-chokepoint/semantic-task-contract-v1.6.json` |
| Runtime Prompt | 独立`segment-fact-extractor-v1.6.md`；模型只抽Qualification事实，代码机械评分 |
| 验证状态 | Golden、Holdout、HBM边界回归与Top-10复核均未重跑 |

本文件不修改或回写v1.5及更早版本。R5/R5.1/R5.2的Evidence Pack、标注、哈希与原语义保持不变；R6必须使用新的合同ID与哈希。

Company三族评分、Replacement、Earnings以及未被本文件替换的Segment维度、Bound Basis、Evidence Capability、Coverage、Hard Gate和状态优先级继续继承v1.5。Qualification Barrier以本合同第4A节为准。

v1.6机器合同、Prompt、validator和边界测试已经建立，但R6 Golden、Holdout、HBM边界回归与Top-10人工复核尚未完成。因此当前只能是`freeze_candidate`，不得标记为`frozen`或声称已通过生产验收。默认生产路径保持不变；只有显式启用v1.6资产才执行本合同。
...
## 4A. Qualification Barrier双路径

Qualification仍评估替代供应商进入当前原子Scope所需的正式准入障碍，但不再要求所有合法Bound都必须找到精确月数。模型只抽取事实，代码按以下两条路径机械评分。
...
2：>6–≤12个月
3：>12–<18个月
4：≥18个月，且有流程步骤、历史周期或当前里程碑原文
```

若已证明`≥18个月`但完全没有流程或里程碑证据，输出`[3,4]`，不得exact 4。

### 4A.2 公开可观察流程路径

精确时长未公开时，允许以下保守floor：

```text
明确要求正式Qualification、Approved Vendor List、型式试验或客户批准
-> lower_bound [1,4]

至少两个可分离的连续准入步骤，例如设计审查+型式试验、FAT+客户批准
-> lower_bound [2,4]
...
只有流程存在但时长未知时不得生成exact。完全没有Qualification要求、时长或流程证据时仍为`unknown [0,4]`；搜索未发现不等于无需认证。

### 4A.3 禁止代理

交付周期、backlog、排队时间、建厂、设备安装、一般供应紧张和“diversifying sourcing”均不得替代Qualification事实。只有原文明确说明标准/现货供应在当前Scope无需特殊批准，或认证≤3个月，才能形成0档。
...
v1.6拥有独立机器合同、Prompt、Validator和Schema。v1.5及更早版本的文档、Prompt、机器资产、测试和历史结果保持只读；v1.6字段与评分不得反向迁移到旧结果。当前默认生产配置保持不变，只有显式启用v1.6资产才执行Qualification双路径语义。
...
Coverage Receipt必须记录精确commit SHA、clean/dirty状态、v1.6合同版本，以及PRD、机器合同、Prompt、Validator、Golden和Holdout资产SHA。在R6人工Golden/Holdout/HBM/Top-10门未真实通过前，本合同必须保持`freeze_candidate`。
