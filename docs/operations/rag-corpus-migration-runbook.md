...
one HTTP 429, one missing value, one naive value, and one invalid value).

The MCP writer was stopped and the canonical/control/refresh databases were
copied to `.tmp/rag-live-refresh-pre-20260816-004` before the reviewed apply.
The exact-ID apply below completed with seven successes, zero failures, and
`applied_count=7`; its evidence is
`.tmp/publication-backfill-reviewed-seven-apply.json`. Read-only reconciliation
then found 412 receipts for 412 distinct article IDs, 412
`publisher_metadata` rows, nine ready `legacy_unverified` rows, and 70
`legacy_unverified` rows overall. The other nine remain excluded. URL date
segments, prose datelines, the legacy `published_at`, fetch time, and HTTP
`Last-Modified` were not used as substitutes for publisher publication
metadata.
...
The observed postcondition matched the reviewed expectation exactly. On a
future run, a smaller applied count remains a safe partial result, not authority
to fill missing dates from weaker evidence. Reconcile receipt IDs and guarded
row values before building any later successor generation.
...
The earlier live failures are retained as incident evidence rather than hidden:

- `run_9acdd945ff18480ea89d33ae07d58ab7` lost NewsAPI page 1 when page 2
  returned provider code `maximumResultsReached`;
- `run_e450ffa0582b47d79a43636e1b8e2112` reached 92 ready inputs but sent
  duplicate canonical article IDs into the successor proof, so it failed
  before candidate registration; and
- `run_937f8b6eee5f4e06b0774f88dc19a934` completed 92 content successes and
  eight permanent exclusions, then exposed that an existing refresh ledger did
  not gain the new frozen-provider-request column from `CREATE TABLE IF NOT
  EXISTS` alone. It failed before a candidate was built, and the old pointer
  remained active.

The production fixes retain successful NewsAPI pages on the exact provider
terminal condition, deduplicate equal article/hash proof entries while failing
closed on conflicting hashes, and upgrade existing ledger tables in place with
the frozen-request columns. Each semantic correction advanced the refresh scope
contract; the currently executable contract is `refresh-generation-proof-v5`,
so failed v2/v3/v4 gates cannot suppress a corrected root run.

The real HTTP MCP v5 refresh `run_18c0e2fe7c884d45b248200d469f73da`
completed on 2026-08-16 with 100 inputs, 92 content/summary/index-ready and
usable items, eight permanent HTTP 401/403 exclusions, zero retryable failures,
and a persisted 507-article generation proof. It built and atomically activated
`news-successor-7bc5eaa452d144f99932e61cadc7e83f` only after readback verified
507 manifest articles, 3,356 chunks, and zero issues. Its canonical snapshot is
`d33e0aa66f12fb046564cb4337138c73f49fe8a471ffef4094d5bc7174632b0a`.
The predecessor `news-generation-20260815-v1` remains intact as `verified`;
the active pointer now names the successor.

The initial long-running HTTP client was terminated after durable completion
because it continued waiting despite the completed ledger and active pointer;
a second real MCP call returned `already_completed` for the same run in under
two seconds. Treat the durable run/proof/pointer records plus that gate response
as the refresh evidence, not the terminated client's missing terminal display.
An active-generation reader then resolved all 507 articles and 3,356 chunks;
the default 30-day window admitted 90 verified-date articles and returned three
results, all bound to the successor generation and `source_metadata` dates.

A real one-ticker Refresh-to-Watchlist smoke produced run
`a619348a-b901-4507-bb56-dc70b0b5b1e5`, report
`reports/watchlist_triage/2026-08-16_024342_watchlist.md`, and its matching
timeline JSONL. Both files persist the same generation ID, corpus snapshot,
index-config fingerprint, corpus/collection identity, and embedding artifact;
the timeline reader returned 20 events and the complete provenance header.
...
durable Retry-After data and a secret-free frozen provider request rather than
blindly rerunning the root pipeline or recomputing a mutable batch. The MCP owns
one automatic retry poller. Root and child work have independent lease
watchdogs; content and summary adapters re-check the lease after provider return
and before canonical mutation. The parent completion gate uses the latest
terminal child baseline, cumulative accepted/rejected/permanent observations,
latest manifest, attempts, and collection outcomes. Activation uses an
expected-current compare-and-swap inside `BEGIN IMMEDIATE`, so concurrent
successors cannot silently overwrite each other. Grounded non-insufficient
answers require at least one valid inline evidence citation.
