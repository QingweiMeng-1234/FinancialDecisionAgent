...
| Field | Value |
| --- | --- |
| Status | Implementation draft aligned to frozen v1.4 scoring contract |
| Date | 2026-08-15 |
...
| Frozen scoring contract | `docs/product/theme-chokepoint-scoring-contract-v1.4.zh-CN.md` |
| Historical scoring contract | `docs/product/theme-chokepoint-scoring-contract-v1.zh-CN.md` |
| Boundary-regression evidence | Historical v1.1 report plus the sealed v1.4 HBM and v0.6 Holdout assets; none of these alone proves runtime conformance or live E2E behavior |
...
5. The system mechanically records `assessment_revised`, `strengthening`, `weakening`, `invalidated`, `transferred`, `coexisting_constraints`, or `unchanged` from comparable snapshots and evidence-bound Gate changes.
...
Every candidate segment shall be evaluated using the normative `theme-chokepoint-scoring-v1.4` contract. The v1.4 Segment dimensions and weights are inherited from v1.2:
...
The v1.4 `candidate_chokepoint` gate is:
...
For incumbents and current bottleneck owners, the system shall calculate a separate 100-point defensibility range using the v1.4 weights inherited from v1.2:
...
For challengers, substitute suppliers, and alternative routes, the system shall calculate a separate 100-point replacement-momentum range using the v1.4 weights inherited from v1.2:
...
The system shall calculate a separate 100-point earnings-transmission range using the v1.4 weights inherited from v1.2 and the v1.4 concentration/protection split:
...
| Customer Concentration Exposure | 2.5 | Named-customer revenue concentration is measured separately from contractual protection. |
| Customer Relationship Protection | 2.5 | Contract duration, cancellation rights, inventory obligations, and switching protection constrain bargaining risk. |
...
The frozen normative scoring baseline is `theme-chokepoint-scoring-v1.4`. It inherits the v1.2 thresholds, milestones, interval and Coverage semantics and the v1.3 mechanical validation boundary, then splits customer concentration from relationship protection, separates product/service Replacement profiles, and requires explicit `bound_basis`. The uncontaminated v0.6 Holdout passed every pre-adjudication gate before HBM and Top-10 validation were run.
...
The legacy `true/false/unknown/conflicted` feedback shape is not valid for v1.4 score dimensions and shall not be emitted. Feedback shall reference stable run and object IDs.
...
New runs shall report state changes as `assessment_revised`, `strengthening`, `weakening`, `invalidated`, `transferred`, `coexisting_constraints`, or `unchanged`, with the evidence responsible for the change.

`knowledge_revision` shall always produce `assessment_revised`, never a business trend. `transferred` requires evidence that the former constraint is easing, an adjacent upstream or downstream node is strengthening as a constraint, and the underlying demand has not simply disappeared. If both the original and adjacent nodes remain constrained, the state is `coexisting_constraints`.
  ...
  "dimensions": [
    {
      ...
      "condition_coverage": {
        ...
        "resolved_required_condition_count": 3
      }
    },
    {
...
- the frozen v1.4 contract has passed all HBM boundary assertions plus cross-theme positive-case, double-annotation, adjudication, and calibration Gates; runtime conformance is a separate exit criterion;
...
- Stage 5 writes its manifest last as a recoverable commit marker; this is not a claim that the output directory is atomically published.
...
- Stage 7 writes its export manifest last as a recoverable commit marker; readers must ignore or clean partial directories without a valid manifest.
...
The approved normative boundary-regression case for `theme-chokepoint-scoring-v1.4` is:
...
- the historical `hbm-advanced-packaging-golden-regression-v1.1-2026-08-15.zh-CN.md` report and the sealed v1.4 HBM regression report under the v0.6 freeze assets.
...
The required cross-theme validation case is AI data-center power and cooling infrastructure, with global and regional power, interconnection, transformer/switchgear, backup-power, and cooling constraints kept as separate scopes. Its v0.6 double annotation and Top-10 review are sealed freeze evidence for v1.4; they do not prove that the Stage 1–7 runtime executes the frozen contract.
...
The v1.4 scoring contract is the frozen **human-readable scoring-semantics baseline** as of 2026-08-16. Its numerical weights, anchors, intervals, Coverage formulas, candidate gates, multi-axis milestones, relief model, and state priorities inherit v1.2; v1.3 supplies complete weighted-dimension tasks, evidence capabilities, and mechanical recomputation; v1.4 adds split concentration/protection dimensions, product/service Replacement profiles, and mandatory `bound_basis`. The machine artifact whose SHA was sealed by the freeze manifest is currently unavailable, so the present v1.4.1 runtime overlay is not a reproducible frozen executable baseline. Any later semantic change requires a new scoring-contract version and renewed Golden, Holdout, and Top-10 validation.
...
The historical HBM/advanced-packaging 22-item boundary regression, uncontaminated v0.6 double-blind gate, and AI data-center power/cooling Top-10 review passed and freeze the v1.4 human scoring semantics. They do not prove that the current runtime overlay reproduces the missing frozen machine artifact. Before live-provider E2E or ranking acceptance, reviewers must approve the executable and delivery boundaries below:
...
- `theme-chokepoint-scoring-v1.4` as the frozen human scoring-semantics baseline, with the current executable overlay separately versioned and unfrozen;
...
## v1.5 Freeze-Candidate Addendum（2026-08-20）

本Addendum不修改v1.4冻结基线。v1.5的规范来源为`theme-chokepoint-scoring-contract-v1.5.zh-CN.md`和独立机器合同`semantic-task-contract-v1.5.json`；若本PRD旧章节与其冲突，只有显式启用v1.5的运行使用新语义，v1.4历史运行继续按原SHA解释。

v1.5关闭以下产品缺口：Demand Pressure使用同口径年增长档；Downstream Criticality采用永久100%结构测试与Top1停供90天运营测试；Effective Supply Concentration冻结有效产出分母、30/30/40合成与90天增量Failover；Capacity Inelasticity绑定残余`ΔQ`、物理DAG关键路径及连续3个月稳定良品；Substitute Weakness冻结异路线集合、覆盖去重、两轮收敛和unresolved/budget incomplete规则。

Stage 3仍是company-neutral Segment评分，但Claim/Card可以用`subject_company_ids[]`标注事实主体。Stage 4不得直接拿Stage 3 Claim支持Defensibility、Replacement或Earnings；它必须以不可变SourceVersion和Quote lineage生成新的CompanyScope Claim/Card，并重新完成所需Business Fact能力验证。Stage 4新事实只能发出`segment_recompute_required`，不得修改同一Run的Stage 3结果。

默认v1.5 Company链为：Company Mapping → Challenger Discovery → Route/Company Counter-search → Frozen typed ChallengerSet → Company Evidence/Scoring/Critic → Competition State。每个bottleneck owner原子Scope都必须有ChallengerSet；集合缺失、Coverage Gate未通过、预算耗尽或存在unresolved路线时持久化为`COMPANY_ASSESSMENT_INCOMPLETE`，扣留Competition State并阻止Stage 5。恢复从已持久化进度继续。

v1.5当前状态为`freeze_candidate`。只有机器合同、Prompt、Validator、边界测试、Golden、未污染Holdout、HBM/先进封装与跨行业回归、Top-10复核和Coverage Receipt均满足合同门槛后，才允许另行提交状态变更为`frozen`。
