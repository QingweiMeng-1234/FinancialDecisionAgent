# Theme Chokepoint Prompt Workflow Validation Experiment v0.2

```text
experiment_id: TC-PROMPT-WF-VAL-001
round: 2
core_prompt_revision: TC-SESSION-PROMPT-v2.2
core_prompt_sha256: 52c9abbe0f4a10c5521464438ae5d35ad7e6575cf371feb4ab421011dd301d21
...
- Round 2 不得收到 Round 1 程序、报告、finding、receipt、Git history、实现解释或目录路径；只允许收到 v2.2 核心 Prompt、本 v0.2 适配层和新的空 projectless 目录。
...
12. validator 在 Initial Review 前通过 v2.2 第 13.3 节全部 conformance cases；
...
16. 若 review control plane 失败，只能走 v2.2 第 13.4 节受限 recovery/replay。
...
只有 16 项 Workflow 验证要求全部满足、程序验收全部通过、且至少一个真实 Remediation Loop 被完整遍历，才可输出 `EXPERIMENT_WORKFLOW_VALIDATED`。
...
这是 Round 2 clean-room rerun。Round 1 只用于 Prompt Owner 决定重启，不是本 Session 的授权输入或先验 finding。不得预先假定核心 Prompt 有效，也不得为了结束实验而输出 GO。
