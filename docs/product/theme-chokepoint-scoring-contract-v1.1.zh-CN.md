# Theme Chokepoint Scoring Contract v1.1
...
| 状态 | `freeze_candidate`；HBM边界回归已通过，跨主题正例校准与双标注审查未完成 |
| 决策日期 | 2026-08-15 |
| 适用产品 | Theme Chokepoint Research Agent |
| 替代版本 | v1.1生效后替代`theme-chokepoint-scoring-v1`；v1保留为历史合同 |
| 决策记录 | `theme-chokepoint-scoring-v1.1-decision-baseline.zh-CN.md` |
...
本文是v1.1设计候选中唯一的评分语义来源，但在重新冻结前不得被称为已批准实现基线。PRD、v1、旧Golden或讨论稿与本文候选语义冲突时，以本文记录当前候选；实现授权仍以重新冻结Gate为准。
...
Scope任一关键字段不兼容时必须拆卡，禁止跨产品、客户、平台、地区或时期拼接评分。
...
总分和Coverage不能补偿Hard Gate的`fail/unknown`。
...
| Customer Adoption | 客户拒绝、取消、移除或停止生产使用 | 付费评估、试点或初始商业订单 | 一个目标客户/平台生产使用或重复小批量 | 具名平台跨两个周期持续使用，或多个客户生产采用 | 多客户/多平台规模化持续生产采用 |
...
`replacement_state_eligible=true`要求Scope与时效有效、无`evidence_mapping_error`并通过第10.1节Replacement Discovery Gate；否则`primary_state=null`并给出`withheld_reason`。
...
输出累计`achieved_gates[]`和唯一`primary_state`，优先级：Scaled Replacement → Realized → Scaled Alternative → Production → Ready → Credible → Early Signal。`production_alternative`继承Ready；Realized还执行更高的六项不可补偿门，因此Performance、Qualification、Capacity、Adoption、Ecosystem或Displacement任一未达到3时，总分和收入均不能补偿。
