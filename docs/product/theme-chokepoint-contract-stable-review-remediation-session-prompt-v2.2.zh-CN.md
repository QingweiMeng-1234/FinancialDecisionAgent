...
# Contract-Stable Architecture、Implementation、Review、Challenge 与 Exit 完整 Session Prompt v2.2

> revision_id: TC-SESSION-PROMPT-v2.2

> 用途：将本文件完整粘贴给一个新的总控 Session，从零启动或从已经有效冻结的 Architecture Program 启动 Theme Chokepoint 的设计、实现、独立审核和退出闭环。
>
> 本 Prompt 取代旧“五轮上限独立审查、修复、挑战与退出闭环”Prompt、允许 Orchestrator 兼任 Developer 的 v2 调度语义及 v2.1 的单槽 adjudicator/事后 sidecar/无控制面恢复语义，但不改写旧 Prompt、历史 Cycle、finding、receipt、Golden、Holdout 或 root-cause history。
...
在 Initial Reviewer 启动前，Developer 必须对 validator 留下真实 RED→GREEN，并至少用 synthetic sealed receipts 验证：
...
缺少任一 conformance case 时，implementation 不得进入 Initial Review；现有 unit tests 绿色不能替代该矩阵。
...
9. 所有运行代码严格 TDD；在 Initial Review 前先通过第 13.3 节 validator conformance suite；
...
18. 所有角色在离场前自行封存 canonical JSON receipt，禁止 Orchestrator 事后转录；
19. validator 自身缺陷只允许走第 13.4 节一次性同轮 control-plane repair/replay，不得伪造合法 Cycle Decision。
